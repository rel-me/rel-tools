...
`rel --api` exists only in the proprietary binary bundled with Rel.app. It is
not part of the public `rel-cli` package.

`health` and `status` inspect the currently running API service without launching the
app. Every other command, including `mcp`, proxy reads, and session reads,
starts Rel.app in the background when its API service is unavailable. `rel mcp`
performs that startup check once, then serves its original stdio connection.
`REL_API_PORT` overrides the default local port, `17319`.
...
If a browser operation is still active, the resident API service cancels the matching
Chromium work; Rel.app, the API service, and the persistent browser session remain
