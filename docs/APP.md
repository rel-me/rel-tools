...
When REL ends browser work to stay within a response budget or retry limit,
the final message includes a **REL stop reason** footer generated by REL. Token
budget stops show recorded provider usage, estimated next-input tokens, the
answer reserve, and the response limit. Model-call and retry limits have distinct
