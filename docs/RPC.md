...
`POST /v1/proxy-transfers/export` accepts an `alias` and
`include_password` boolean. It returns `data.filename` and `data.contents`,
where `contents` is the complete versioned `.relproxy` JSON text. Ordinary
clients must set `include_password:false`. Including the app-protected password
also requires the private authorization header held by the owning REL app.
`POST /v1/proxy-transfers/import` accepts `contents` and an optional `alias`
override, validates the complete transfer in Rust, and returns the newly
created `data.proxy`. Transfers are limited to 1 MiB and never overwrite an
existing alias.
...
`POST /v1/profile-transfers/export` accepts a profile `name` and returns
`data.filename` plus the complete versioned `.relprofile` JSON in
`data.contents`. `POST /v1/profile-transfers/import` accepts `contents` and an
optional `name` override, validates the complete transfer in Rust, and returns
the newly created `data.profile`. Transfer files are limited to 1 MiB and
exclude app-owned cookie and saved-password values; imports always create a
custom profile with no browser data and never overwrite an existing name.
