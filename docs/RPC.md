...
- `DELETE /v1/proxies/{alias}` detaches it from all sessions, then returns
  `data.deleted_alias`.
- `POST /v1/proxies/{alias}/rotate-session` requires an Oxylabs-enabled proxy,
  replaces the session-owned sticky ID for each open session assigned to it,
  and returns `data.proxy`.
...
or `st`. Proxy resources do not contain sticky session state.
...
- `POST /v1/sessions` accepts optional `name`, `group`, `profile`, `proxy_alias`,
  ...
  `data.session`. When the effective proxy is Oxylabs-enabled, REL generates a
  fresh sticky ID owned by that new session.
...
Oxylabs sticky IDs are stored internally with their browser sessions and are
not returned in session or proxy resources.
